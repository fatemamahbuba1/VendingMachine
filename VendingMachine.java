...
public class VendingMachine {
        ...
        for (int row = 0; row < 6; row++) {
            for (int col = 0; col < 6; col++) {
                if (inventory.isItemAvailable(row, col)) {
                    ItemCategory item = inventory.getItemType(row, col);
    ...
    public void selectItem(int row, int col) {
        if (inventory.isItemAvailable(row, col)) {
            ItemCategory selectedItem = inventory.getItemType(row, col);
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            System.out.print("Enter your selection (e.g. 1 2) or 0 to exit: ");
        ...
        System.out.println("Total earned amount: $" + vendingMachine.getTotalAmount());
        scanner.close();
    }
}
